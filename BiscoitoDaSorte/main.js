// Como estou com pouco tempo venho reutilizando bastante os códigos anteriores.

// Variáveis

const screen1 = document.querySelector('.screen1')
const screen2 = document.querySelector('.screen2')
const buttonTry = document.querySelector('#cookie')
const buttonRetry = document.querySelector('#retryButton')
let randomNumber = Math.round(Math.random() * 61)

// Funções

function handleCookieClick(event) {
    event.preventDefault() // Remova isso e o argumento evento e o segundo if da handleEnterKey.
    document.querySelector('.fortune p').innerText = fortune[randomNumber] // Altera a dom usando um índice aleatório para a lista.
    handleRetryClick()
}

function handleRetryClick() {   
    toggleScreen()
    randomNumber = Math.round(Math.random() * 61) // Reseta o número a cada tentativa.
}

function handleEnterKey(pressed) {
    if(pressed.key == 'Enter' && screen1.classList.contains('hide')){
        handleRetryClick()
    }
    /* if(pressed.key == 'Enter' && screen2.classList.contains('hide')){
        handleCookieClick()
    } */
}
...
let fortune = [
    'Procure acenderu ma vela ao invés de amaldiçoar a escuridão.',
